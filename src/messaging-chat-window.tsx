...
import MessagingChatMessage, { MessagingChatMessageProps } from "./messaging-chat-message";
...
const MessagingChatWindow = React.forwardRef<HTMLDivElement, MessagingChatWindowProps>(
  ({paginate, toggleMessagingProfileSidebar, ...props}, ref) => {
    const [messagingChatConversations, setMessagingChatConversations] = useState<MessagingChatMessageProps[]>([])
    return (
      <div ref={ref} {...props}>
        <div className="w-full flex-col sm:border-default-200 lg:border-l-small xl:border-r-small">
          ...
          <div className="flex w-full overflow-visible">
            <ScrollShadow className="flex max-h-[calc(100vh-220px)] flex-col gap-6 px-6 py-4 lg:max-h-[calc(100vh-162px)] w-full min-h-[800px] ">
          ...
          <div className="mx-2 mt-auto flex flex-col">
            <MessagingChatInput addMessage={(msg: MessagingChatMessageProps) => setMessagingChatConversations((_prev) => [..._prev, msg])} messagingChatConversations={messagingChatConversations} />
          </div>
        </div>
      </div>
    );
